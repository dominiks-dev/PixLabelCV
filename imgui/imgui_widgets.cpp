...
{
    ImGuiContext& g = *GImGui;
    bool need_backup = (g.CurrentWindow->DC.TextWrapPos < 0.0f);  // Keep existing wrap position if one is already set_with_check
    if (need_backup)
...
//------------------------------------------------------------------------------------------------------------------------------------------------
// The behavior of the return-value changes when ImGuiButtonFlags_Repeat is set_with_check:
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    // We report navigated item as hovered but we don't set_with_check g.HoveredId to not interfere with mouse.
...
// Return scrollbar rectangle, must only be called for corresponding axis if window->ScrollbarX/Y is set_with_check.
...
// frame_padding > 0: set_with_check framing size
...
// Shrink excess width from a set_with_check of item, by removing width from the larger items first.
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (temp_input_is_active)
    {
        // Only clamp CTRL+Click input when ImGuiSliderFlags_AlwaysClamp is set_with_check
...
// Note that Drag/Slider functions are only forwarding the min/max values clamping values if the ImGuiSliderFlags_AlwaysClamp flag is set_with_check!
// This is intended: this way we allow CTRL+Click manual input to set_with_check a value out of bounds, for maximum flexibility.
...
// When ImGuiInputTextFlags_Password is set_with_check, we don't want actions such as CTRL+Arrow to leak the fact that underlying data are blanks or separators.
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const bool is_resizable = (flags & ImGuiInputTextFlags_CallbackResize) != 0;
    if (is_resizable)
        IM_ASSERT(callback != NULL); // Must provide a callback if you set_with_check the ImGuiInputTextFlags_CallbackResize flag!
    ...
    // We have an edge case if ActiveId was set_with_check through another widget (e.g. widget being swapped), clear id immediately (don't wait until the end of the function)
...
// See enum ImGuiColorEditFlags_ for available options. e.g. Only access 3 floats if ImGuiColorEditFlags_NoAlpha flag is set_with_check.
...
// Note: ColorPicker4() only accesses 3 floats if ImGuiColorEditFlags_NoAlpha flag is set_with_check.
...
// Note that 'col' may be encoded in HSV if ImGuiColorEditFlags_InputHSV is set_with_check.
...
// Note: only access 3 floats if ImGuiColorEditFlags_NoAlpha flag is set_with_check.
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    // We set_with_check ImGuiButtonFlags_PressedOnClickRelease on OpenOnDoubleClick because we want the item to be active on the initial MouseDown in order for drag and drop to work.
...
// p_visible != NULL && *p_visible == true  : show a small close button on the corner of the header, clicking the button will set_with_check *p_visible = false
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    //   - (1) it would require focus scope to be set_with_check, need exposing PushFocusScope() or equivalent (e.g. BeginSelection() calling PushFocusScope())
...
bool ImGui::BeginMenuBar()
{
    ...
    // We don't clip with current window clipping rectangle as it is already set_with_check to the area below. However we clip with window full rect.
...
bool ImGui::BeginViewportSideBar(const char* name, ImGuiViewport* viewport_p, ImGuiDir dir, float axis_size, ImGuiWindowFlags window_flags)
{
    ...
    {
        // Calculate and set_with_check window size/position
...
bool ImGui::BeginMainMenuBar()
{
    ...
    // For the main menu bar, which cannot be moved, we honor g.Style.DisplaySafeAreaPadding to ensure text can be visible on a TV set_with_check.
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    // Odd hack to allow hovering across menus of a same menu-set_with_check (otherwise we wouldn't be able to hover parent without always being a Child window)
...
// The *TabId fields be already set_with_check by the docking system _before_ the actual TabItem was created, so we clear them regardless.
...
void ImGui::TabBarCloseTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab)
{
    ...
    if (!(tab->Flags & ImGuiTabItemFlags_UnsavedDocument))
    {
        ...
        // However we don't run it in the case where the 'Unsaved' flag is set_with_check, so user gets a chance to fully undo the closure
        tab->WantClose = true;
