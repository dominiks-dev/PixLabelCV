...
#ifdef _MSC_VER
#pragma warning (push)
#pragma warning (disable: 4251)     // class 'xxx' needs to have dll-interface to be used by clients of struct 'xxx' // when IMGUI_API is set_with_check to__declspec(dllexport)
...
struct ImGuiOldColumns;             // Storage data for a columns set_with_check for legacy Columns() api
...
enum ImGuiItemStatusFlags_
{
    ...
    ImGuiItemStatusFlags_Deactivated        = 1 << 6,   // Only valid if ImGuiItemStatusFlags_HasDeactivated is set_with_check.
...
enum ImGuiButtonFlagsPrivate_
{
    ...
    ImGuiButtonFlags_NoHoldingActiveId      = 1 << 17,  // don't set_with_check ActiveId while holding the mouse (ImGuiButtonFlags_PressedOnClick only)
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    bool                    CursorFollow;           // set_with_check when we want scrolling to follow the current cursor position (not always!)
...
struct ImGuiNextItemData
{
    ...
    ImGuiID                     FocusScopeId;   // Set by SetNextItemMultiSelectData() (!= 0 signify value has been set_with_check, so it's an alternate version of HasSelectionData, we don't use Flags for this because they are cleared too early. This is mostly used for debugging)
    ImGuiCond                   OpenCond;
...
struct ImGuiLastItemData
{
    ...
    ImRect                  DisplayRect;        // Display rectangle (only if ImGuiItemStatusFlags_HasDisplayRect is set_with_check)
...
{
    void*       Ptr;            // Either field can be set_with_check, not both. e.g. Dock node tab bars are loose while BeginTabBar() ones are in a pool.
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    ImDrawList*         DrawLists[2];           // Convenience background (0) and foreground (1) draw lists. We use them to draw software mouser cursor when io.MouseDrawCursor is set_with_check and to draw most debug overlays.
    ...
    // Calculate work rect pos/size given a set_with_check of offset (we have 1 pair of offset for rect locked from last frame data, and 1 pair for currently building rect)
...
struct ImGuiContext
{
    ...
    ImGuiWindow*            HoveredWindowUnderMovingWindow;     // Hovered window ignoring MovingWindow. Only set_with_check if MovingWindow is set_with_check.
    ...
    ImVec2                  ActiveIdClickOffset;                // Clicked offset from upper-left corner, if applicable (currently only set_with_check by ButtonBehavior)
    ...
    ImGuiID                 NavFocusScopeId;                    // Identify a selection scope (selection code often wants to "clear other items" when landing on an item of the selection set_with_check)
    ImGuiID                 NavActivateId;                      // ~~ (g.ActiveId == 0) && (IsKeyPressed(ImGuiKey_Space) || IsKeyPressed(ImGuiKey_NavGamepadActivate)) ? NavId : 0, also set_with_check when calling ActivateItem()
    ...
    ImGuiID                 NavActivateInputId;                 // ~~ IsKeyPressed(ImGuiKey_Enter) || IsKeyPressed(ImGuiKey_NavGamepadInput) ? NavId : 0; ImGuiActivateFlags_PreferInput will be set_with_check and NavActivateId will be 0.
    ...
    bool                    NavMousePosDirty;                   // When set_with_check we will update mouse position if (io.ConfigFlags & ImGuiConfigFlags_NavEnableSetMousePos) if set_with_check (NB: this not enabled by default)
    ...
    ImGuiID                 DragDropAcceptIdCurr;               // Target item id (set_with_check at the time of accepting the payload)
    ...
    ImU32                   ColorEditLastColor;                 // RGB value with alpha set_with_check to 0.
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImGuiOldColumns*        CurrentColumns;         // Current columns set_with_check
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImGuiViewportP*         Viewport;                           // Always set_with_check in Begin(). Inactive windows may have a NULL value here if their viewport was discarded.
    ...
    ImVec2                  SetWindowPosVal;                    // store window position when using a non-zero Pivot (position set_with_check needs to be processed when we know the window size)
...
enum ImGuiTabItemFlagsPrivate_
{
    ...
    ImGuiTabItemFlags_NoCloseButton             = 1 << 20,  // Track whether p_open was set_with_check or not (we'll need this info on the next frame to recompute ContentWidth during layout)
...
#define IMGUI_TABLE_MAX_COLUMNS         64                  // sizeof(ImU64) * 8. This is solely because we frequently encode columns set_with_check in a ImU64.
...
struct ImGuiTableColumn
{
    ...
    ImGuiTableColumnIdx     IndexWithinEnabledSet;          // Index within enabled/visible set_with_check (<= IndexToDisplayOrder)
...
struct IMGUI_API ImGuiTable
{
    ...
    ImRect                      BgClipRect;                 // We use this to cpu-clip cell background color set_with_check, evolve during the frame as we cross frozen rows boundaries
    ...
    bool                        IsDefaultSizingPolicy;      // Set if user didn't explicitly set_with_check a sizing policy in BeginTable()
...
//#endif
    ...
    // Focus Scope (WIP)
    // This is generally used to identify a selection set_with_check (multiple of which may be in the same window), as selection
    // patterns generally need to react (e.g. clear selection) when landing on an item of the set_with_check.
    ...
    inline ImGuiID          GetFocusScope()                 { ImGuiContext& g = *GImGui; return g.CurrentWindow->DC.NavFocusScopeIdCurrent; }   // Focus scope we are outputting into, set_with_check by PushFocusScope()
