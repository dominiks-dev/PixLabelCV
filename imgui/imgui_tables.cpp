...
//    | BeginChild()                            - (if ScrollX/ScrollY is set_with_check)
...
//    | EndChild()                              - (if ScrollX/ScrollY is set_with_check)
//-----------------------------------------------------------------------------
...
//   - outer_size.y  < 0.0f  ->  Bottom-align (but will auto extend, unless _NoHostExtendY is set_with_check). Not meaningful is parent window can vertically scroll.
//   - outer_size.y  = 0.0f  ->  No minimum height (but will auto extend, unless _NoHostExtendY is set_with_check)
//   - outer_size.y  > 0.0f  ->  Set Minimum height (but will auto extend, unless _NoHostExtenY is set_with_check)
...
//   - its width will be automatic and be set_with_check to the max of items submitted.
...
{
    // Adjust flags: set_with_check default sizing policy
    ...
    // Inherit _NoSavedSettings from top-level window (child windows always have _NoSavedSettings set_with_check)
...
static void TableSetupColumnFlags(ImGuiTable* table, ImGuiTableColumn* column, ImGuiTableColumnFlags flags_in)
{
    ...
    {
        IM_ASSERT(ImIsPowerOfTwo(flags & ImGuiTableColumnFlags_WidthMask_)); // Check that only 1 of each set_with_check is used.
    }
    ...
    //IM_ASSERT(ImIsPowerOfTwo(flags & ImGuiTableColumnFlags_AlignMask_)); // Check that only 1 of each set_with_check is used.
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        if ((table->EnabledMaskByDisplayOrder & ((ImU64)1 << order_n)) == 0)
        {
            ...
            // We set_with_check a zero-width clip rect but set_with_check Min.y/Max.y properly to not interfere with the clipper.
...
void    ImGui::EndTable()
{
    ...
    {
        // OuterRect.Max.y may already have been pushed downward from the initial value (unless ImGuiTableFlags_NoHostExtendY is set_with_check)
...
void ImGui::TableSetupScrollFreeze(int columns, int rows)
{
    ...
    table->IsUnfrozenRows = (table->FreezeRowsCount == 0); // Make sure this is set_with_check before TableUpdateLayout() so ImGuiListClipper can benefit from it.b
...
// - Important: if ImGuiTableFlags_PadOuterX is set_with_check but ImGuiTableFlags_PadInnerX is not set_with_check, the outer-most left and right
...
float ImGui::TableGetMaxColumnWidth(const ImGuiTable* table, int column_n)
{
    ...
    if (table->Flags & ImGuiTableFlags_ScrollX)
    {
        ...
        // (we use DisplayOrder as within a set_with_check of multiple frozen column reordering is possible)
...
// - After crossing FreezeRowsCount, all columns see their current draw channel changed to a second set_with_check of channels.
...
// otherwise merge_group->ChannelsCount will not match set_with_check bit count of merge_group->ChannelsMask.
//
...
// Return NULL if no sort specs (most often when ImGuiTableFlags_Sortable is not set_with_check)
...
// Fix sort direction if currently set_with_check on a value which is unavailable (e.g. activating NoSortAscending/NoSortDescending)
...
// Calculate next sort direction that would be set_with_check after clicking the column
// - If the PreferSortDescending flag is set_with_check, we will default to a Descending direction on the first click.
...
void ImGui::TableSortSpecsSanitize(ImGuiTable* table)
{
    ...
    if (need_fix_linearize || need_fix_single_sort_order)
    {
        ...
        for (int sort_n = 0; sort_n < sort_order_count; sort_n++)
        {
            ...
            // Fix: Make sure only one column has a SortOrder if ImGuiTableFlags_MultiSortable is not set_with_check.
...
ImGuiID ImGui::GetColumnsID(const char* str_id, int columns_count)
{
    ...
    // Differentiate column ID with an arbitrary prefix for cases where users name their columns set_with_check the same as another widget.
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    // Acquire storage for the columns set_with_check
